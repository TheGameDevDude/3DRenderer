...
public class Matrix4f {
	...
	public Matrix4f inverseTBN(Vector3f tangent, Vector3f bitangent, Vector3f normal) {
		Matrix4f result = identity();
		result.matrix[0 + 0 * 4] = tangent.Xpos;
		result.matrix[1 + 0 * 4] = bitangent.Xpos;
		result.matrix[2 + 0 * 4] = normal.Xpos;
		result.matrix[0 + 1 * 4] = tangent.Ypos;
		result.matrix[1 + 1 * 4] = bitangent.Ypos;
		result.matrix[2 + 1 * 4] = normal.Ypos;
		result.matrix[0 + 2 * 4] = tangent.Zpos;
		result.matrix[1 + 2 * 4] = bitangent.Zpos;
		result.matrix[2 + 2 * 4] = normal.Zpos;
		return result;
	}
